...
def k_bound_latex_name(model_name = None, title = False):
    
    if model_name[0] == 'k' and model_name !='k-bound':
        #k_bound_latex = r"$k_{" + str(model_name[1]) + "}$"
...
def dynamic_forecast_ts_plot(normalized_ts_dict = None, title_str = None, plot_file = None, ylabel = None, xlabel = 'time', fontsize = 30, xticks = None, ylim = None, DEFAULT_ALPHA = 1.0, legend_present = False):

    # dictionary:
    # key = ts_name, value is a dict, value = {'xvec': , 'ts_vector', 'style_dict'}

    for ts_name, ts_data_dict in normalized_ts_dict.iteritems():
        ...
        if 'xvec' in ts_data_dict.keys():
            ...
            if 'marker' in ts_data_dict.keys():
                plt.plot(ts_data_dict['xvec'], ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, marker = ts_data_dict['marker'], ls = ts_data_dict['linestyle'], alpha = alpha, color = ts_data_dict['color'])
            else:
                plt.plot(ts_data_dict['xvec'], ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, ls = ts_data_dict['linestyle'], alpha = alpha, color = ts_data_dict['color'])

        else:
            if 'marker' in ts_data_dict.keys():
                plt.plot(ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, marker = ts_data_dict['marker'], ls = ts_data_dict['linestyle'], alpha = alpha, color = ts_data_dict['color'])
            else:
                plt.plot(ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, ls = ts_data_dict['linestyle'], alpha = alpha, color = ts_data_dict['color'])
...
def overlaid_ts(normalized_ts_dict = None, title_str = None, plot_file = None, ylabel = None, xlabel = 'time', fontsize = 30, xticks = None, ylim = None, DEFAULT_ALPHA = 1.0, legend_present = True, DEFAULT_MARKERSIZE = 15):
    ...
    for ts_name, ts_data_dict in normalized_ts_dict.iteritems():
        ...
        if 'xvec' in ts_data_dict.keys():
            ...
            if 'marker' in ts_data_dict.keys():
                plt.plot(ts_data_dict['xvec'], ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, marker = ts_data_dict['marker'], ls = ts_data_dict['linestyle'], alpha = alpha, ms = DEFAULT_MARKERSIZE)
            else:
                plt.plot(ts_data_dict['xvec'], ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, ls = ts_data_dict['linestyle'], alpha = alpha)

        else:
            if 'marker' in ts_data_dict.keys():
                plt.plot(ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, marker = ts_data_dict['marker'], ls = ts_data_dict['linestyle'], alpha = alpha, ms = DEFAULT_MARKERSIZE)
            else:
                plt.plot(ts_data_dict['ts_vector'], lw= ts_data_dict['lw'], label = ts_name, ls = ts_data_dict['linestyle'], alpha = alpha)
