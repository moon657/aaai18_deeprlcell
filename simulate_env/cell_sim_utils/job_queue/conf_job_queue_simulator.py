...
from reward_computation_utils import get_burstProb_actions
...
class ConfRandomForestTimeVariantSimulatorEnv(gym.Env):
    ...
    def __init__(self, experiment_config = None, file_path_config = None, experiment_settings = None):
        ...
        if(self.env_type == 'random_forest'):
            random_forest_model = experiment_config.get('RANDOM_FOREST_PARAMS', 'random_forest_model')
            # pkl file that maps state to B
            self.rf = load_saved_rf(random_forest_model)
