...
class CriticNetwork:
	...
	def create_q_network(self, state_dim, action_dim):
		...
		W3 = tf.Variable(tf.random_uniform([layer2_size,1],-3e-4,3e-4))
	...
	def load_network(self, episode, file_path='saved_critic_networks'):
		self.saver = tf.train.Saver()
		checkpoint = tf.train.get_checkpoint_state(file_path)
		if episode is None:
			checkpoint_path = checkpoint.model_checkpoint_path
		else:
			checkpoint_path = '%s/critic-network-%d' % (file_path, episode)
		self.saver.restore(self.sess, checkpoint_path)

	def save_network(self, episode, file_path='saved_critic_networks'):
		self.saver = tf.train.Saver()
		print 'save critic-network-%d' % episode
		self.saver.save(self.sess, file_path + '/critic-network', global_step=episode)
