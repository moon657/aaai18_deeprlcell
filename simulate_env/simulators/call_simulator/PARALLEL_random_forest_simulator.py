""" Run several experiments in parallel for various parameters 
    of the random forest cell simulator
"""
import sys, os
import numpy as np
from os import path
...
from generate_experiment_params import generate_experiment_params,get_best_experiments_to_replicate
...
def parse_args():
    ...
    parser.add_argument(
        '--predefined_experiments_mode',
        type=str,
        default='TRUE'
    )

    parser.add_argument(
        '--predefined_experiments_file',
        type=str,
        default='/Users/csandeep/Documents/work/uhana/work/YOGURTLAND_EXPERIMENTS/RL_results/beta/subset_RF_cell_sim/NONZERO_ACTION_EXPERIMENTS.txt'
    )
...
if __name__ == "__main__":
    ...
    # ONLY run these experiment settings
    predefined_experiments_file = args.predefined_experiments_file
    # ONLY run these experiment settings
    predefined_experiments_mode = args.predefined_experiments_mode
    ...
    if predefined_experiments_mode == 'FALSE':
        print('all experiments no bias')
        experiment_params_list = generate_experiment_params(TOTAL_EPISODES = TOTAL_EPISODES, TEST_TERM = TEST_TERM, TEST_EPISODES = TEST_EPISODES, train_days = train_days, test_days = test_days, cell_id = cell_id, experiment_prefix = experiment_prefix, median_cell_thpt = median_cell_thpt, timeseries_dir = timeseries_dir)
    else:
        print('predefined experiments mode')
        #def get_best_experiments_to_replicate(experiments_file = None, TOTAL_EPISODES = None, TEST_TERM = None, TEST_EPISODES = None, train_days = None, test_days = None, cell_id = None, experiment_prefix = None, timeseries_dir = None):

        experiment_params_list = get_best_experiments_to_replicate(experiments_file = predefined_experiments_file, TOTAL_EPISODES = TOTAL_EPISODES, TEST_TERM = TEST_TERM, TEST_EPISODES = TEST_EPISODES, train_days = train_days, test_days = test_days, cell_id = cell_id, experiment_prefix = experiment_prefix, timeseries_dir = timeseries_dir)
        #experiment_params_list = get_best_experiments_to_replicate(predefined_experiments_file)

    ## sweep through all experiment parameters
    ###########################################
    #hard_thpt_limit_flag_space = [True]
    ...
    #activity_factor_multiplier_space = [3,5]

    #hard_thpt_limit_space = [.8, 1.0, 1.5, 2.0, 2.5]
    ...
    #delayed_reward_mode = False
    #delay_reward_interval = 1
    ...
    #                    experiment_settings['simulator_mode'] = 'RF'
    ...
    if experiment_run_mode == 'SINGLE':
        ...
        experiment_settings['activity_factor_multiplier'] = 4
        ...
        print('RUN TEST MODE')
        wrapper_run_DDPG_experiment(experiment_settings = experiment_settings, DDPG_dict = DDPG_dict, plotting_info_dict = plotting_info_dict, base_results_dir = base_results_dir, MAST_CELX_fname = MAST_CELX_fname, KPI_list = KPI_list, CELX_df = CELX_df, print_mode = print_mode, try_catch_mode = TRY_CATCH_MODE, env_type = 'random_forest', random_forest_data = random_forest_data)

    # run all experiments across cores
    elif experiment_run_mode == 'PARALLEL':
        d = Parallel(n_jobs=num_cores)(delayed(wrapper_run_DDPG_experiment)(experiment_settings = p, DDPG_dict = DDPG_dict, plotting_info_dict = plotting_info_dict, base_results_dir = base_results_dir, MAST_CELX_fname = MAST_CELX_fname, KPI_list = KPI_list, CELX_df = CELX_df, print_mode = print_mode, try_catch_mode = TRY_CATCH_MODE, env_type = 'random_forest', random_forest_data = random_forest_data) for p in experiment_params_list)

    elif experiment_run_mode == 'SEQUENTIAL':
        for p in experiment_params_list:
            wrapper_run_DDPG_experiment(experiment_settings = p, DDPG_dict = DDPG_dict, plotting_info_dict = plotting_info_dict, base_results_dir = base_results_dir, MAST_CELX_fname = MAST_CELX_fname, KPI_list = KPI_list, CELX_df = CELX_df, print_mode = print_mode, try_catch_mode = TRY_CATCH_MODE, env_type = 'random_forest', random_forest_data = random_forest_data)
